...
# Reproduce by: gnulib-tool --import --dir=. --local-dir=gl --lib=libgnulib --source-base=lib --m4-base=m4 --doc-base=doc --tests-base=gnulib-tests --aux-dir=build-aux --with-tests --avoid=dummy --avoid=lock --libtool --macro-prefix=gl alloca announce-gen argmatch assert calloc canonicalize-lgpl close closeout config-h configmake dirname fdl fsync gettext git-version-gen gitlog-to-changelog gnu-web-doc-update gnumakefile gnupload inttypes lib-ignore long-options lseek maintainer-makefile malloc manywarnings mkstemp mktempd progname realloc rpmatch safe-read stdbool strdup unlink update-copyright useless-if-before-free vc-list-files version-etc-fsf warnings xstrtol
...
libgnulib_la_SOURCES += close-hook.c
...
EXTRA_libgnulib_la_SOURCES += stdio-write.c
